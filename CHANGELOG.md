...
- **HTTP主链路立案每次都重新登录**：`_run_filing()` 每次都执行完整的 Playwright 登录流程（含验证码识别），即使刚登录过且 Token 仍有效。现在先检查 `TokenService` 缓存的 Token，存在则跳过登录直接 HTTP 立案；缓存失效时自动清除并回退完整登录；登录成功后保存 Token 供下次复用

- **CookieService.save() 保存空数据**：`save()` 方法调用 `context.cookies()` 但丢弃返回值，实际写入 `({},)` 空元组。现在正确获取 cookies 并以 `{"cookies": [...]}` 格式持久化，与 `load()` 的读取格式对齐
