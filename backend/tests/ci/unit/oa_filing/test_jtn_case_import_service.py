...
class TestSearchCase:
    @pytest.mark.asyncio
    async def test_empty_case_no_returns_none(self, script: JtnCaseImportScript) -> None:
    ...
    @pytest.mark.asyncio
    async def test_not_found(self, script: JtnCaseImportScript) -> None:
...
class TestSearchCasesByName:
    @pytest.mark.asyncio
    async def test_empty_keyword(self, script: JtnCaseImportScript) -> None:
...
class TestEnsureNameSearchReady:
    @pytest.mark.asyncio
    async def test_sets_flag(self, script: JtnCaseImportScript) -> None:
        with patch.object(script, "_ensure_name_search_playwright_session"):
            await script.ensure_name_search_ready()
...
class TestSearchCases:
    @pytest.mark.asyncio
    async def test_empty_list(self, script: JtnCaseImportScript) -> None:
        results = [item async for item in script.search_cases([])]
        assert results == []

    @pytest.mark.asyncio
    async def test_whitespace_only_filtered(self, script: JtnCaseImportScript) -> None:
        results = [item async for item in script.search_cases(["  ", ""])]
        assert results == []

    @pytest.mark.asyncio
    async def test_http_success(self, script: JtnCaseImportScript) -> None:
