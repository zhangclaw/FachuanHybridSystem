...
def parse_premium_from_response(data: dict[str, Any], institution: str) -> Decimal | None:
    """从响应数据中提取报价金额（纯函数，不依赖 self 或数据库）

    Args:
        data: API 响应数据
        institution: 保险公司代码（仅用于日志，不影响逻辑）
...
def build_premium_request(
    ...
    """构建询价请求的 headers、params、body 和 request_info（纯函数）

    Args:
        premium_query_url: 报价查询 API URL
    ...
    Returns:
        (headers, params, body, request_info) 四元组
    """
...
class InsuranceHttpMixin:
    ...
    def _build_premium_request(
        ...
        return build_premium_request(self.premium_query_url, bearer_token, preserve_amount, institution, corp_id, timeout)

    def _parse_premium_from_response(
        self, data: dict[str, Any], institution: str, elapsed_time: float
    ) -> Decimal | None:
        """从响应数据中提取报价金额"""
        return parse_premium_from_response(data, institution)
    ...
    def _make_failed_result(
        ...
        """构建失败的 PremiumResult"""
        return make_failed_result(company, error_label, exc, request_info, response_data, log_level, extra)
