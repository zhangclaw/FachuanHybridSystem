...
class GuaranteeBaseMixin:  # pragma: no cover
    ...
    def _wait_form_item_option_ready(
        self, *, label_keywords: list[str], option_text: str, timeout_ms: int
    ...
    @staticmethod
    async def _async_random_wait(min_sec: float = 0.5, max_sec: float = 1.3) -> None:
        await asyncio.sleep(random.uniform(min_sec, max_sec))

    async def _async_wait_tree_options_ready(
        self, *, candidates: list[str], timeout_ms: int
    ) -> bool:  # pragma: no cover
        deadline = time.time() + max(timeout_ms, 1000) / 1000
        normalized_candidates = [str(item).strip() for item in candidates if str(item).strip()]
    ...
    async def _async_wait_select_options_ready(
        self, *, candidates: list[str], timeout_ms: int
    ) -> bool:  # pragma: no cover
        deadline = time.time() + max(timeout_ms, 1000) / 1000
        normalized_candidates = [str(item).strip() for item in candidates if str(item).strip()]
    ...
    async def _async_wait_court_options_ready(
        self, *, candidates: list[str], timeout_ms: int
    ) -> bool:  # pragma: no cover
        deadline = time.time() + max(timeout_ms, 1000) / 1000
        normalized_candidates = [str(item).strip() for item in candidates if str(item).strip()]
    ...
    async def _async_wait_form_item_option_ready(
        self, *, label_keywords: list[str], option_text: str, timeout_ms: int
    ) -> bool:  # pragma: no cover
        deadline = time.time() + max(timeout_ms, 1000) / 1000
        cleaned_option = str(option_text or "").strip()
    ...
    async def _async_wait_upload_idle(self, *, timeout_ms: int = 90000) -> bool:  # pragma: no cover
        deadline = time.time() + max(timeout_ms, 1000) / 1000
        while time.time() < deadline:
            uploading = bool(
                await self.page.evaluate(
            ...
            if not uploading:
                return True
            await self._async_random_wait(0.9, 1.4)

        return False

    async def _async_click_first_enabled_button(self, names: list[str]) -> str | None:  # pragma: no cover
        for name in names:
            selectors = [
                self.page.get_by_role("button", name=name),
                self.page.locator("button, [role='button'], .el-button").filter(has_text=name),
    ...
    async def _async_get_visible_form_errors(self) -> list[str]:  # pragma: no cover
    ...
    async def _async_close_popovers(self) -> None:
        for _ in range(2):
            await self.page.keyboard.press("Escape")
            await self._async_random_wait(0.1, 0.2)
