...
class CourtZxfwGuaranteeService(  # pragma: no cover
    GuaranteeDataMixin,
    GuaranteeBaseMixin,
    ...
    async def async_apply_guarantee(self, case_data: dict[str, Any]) -> dict[str, Any]:  # pragma: no cover
        ...
        preserve_category_text = str(case_data.get("preserve_category") or "诉前保全").strip() or "诉前保全"
            ...
            "court": await self._async_choose_court(str(case_data.get("court_name") or "")),
            "preserve_type": await self._async_click_radio_in_form_item(["保全类型"], "财产保全")
            or await self._async_click_radio_by_text("财产保全"),
            "preserve_category": await self._async_click_radio_in_form_item(["保全类别"], preserve_category_text)
            or await self._async_click_radio_by_text(preserve_category_text),
            "case_number": await self._async_fill_case_number(case_data),
            "cause": await self._async_fill_case_cause(
                str(case_data.get("cause_of_action") or ""),
                [str(item) for item in (case_data.get("cause_candidates") or [])],
            ),
            "insurance": await self._async_choose_insurance(insurance_company_name),
            "consultant_code": await self._async_fill_consultant_code(consultant_code),
            "amount": await self._async_fill_amount(case_data.get("preserve_amount")),
            "identity": await self._async_click_radio_in_form_item(["提交人身份"], "律师")
            or await self._async_click_radio_by_text("律师"),
        }

        apply_btn = await self._async_submit_apply_and_wait_g_two()
        await self._async_random_wait(0.8, 1.2)
        ...
        upload_result: dict[str, Any] | None = None
        if "gThree" in self.page.url:
            upload_result = await self._async_complete_g_three(case_data)

        await self._async_advance_to_g_five()

        final_url = self.page.url
        success = "gFive" in final_url
        final_errors = await self._async_get_visible_form_errors() if not success else []
        message = "担保流程执行完成（已到预览页，未提交）"
        if not success:
            message = "担保流程已执行，未到 gFive，请人工确认页面"
            if final_errors:
    ...
    async def _async_advance_to_g_five(self) -> None:
        for _ in range(8):
            if "gFive" in self.page.url:
                return

            if "gThree" in self.page.url:
                errors = await self._async_get_visible_form_errors()
                if any("身份证明材料" in err for err in errors):
                    await self._async_retry_identity_material_upload_in_g_three()
                    await self._async_random_wait(1.4, 2.0)
                if any("请上传证据材料" in err or "证据" in err for err in errors):
                    await self._async_retry_evidence_material_upload_in_g_three()
                    await self._async_random_wait(1.4, 2.0)

            await self._async_click_first_enabled_button(["下一步", "保存并下一步", "暂存"])
            await self._async_random_wait(1.2, 1.8)

    async def _async_submit_apply_and_wait_g_two(self, retries: int = 4) -> str | None:
        last_clicked: str | None = None
        for _ in range(retries):
            last_clicked = await self._async_click_first_enabled_button(["申请担保", "下一步", "保存并下一步"])
            if not last_clicked:
                await self._async_random_wait(0.6, 0.9)
                continue

            for _ in range(10):
                if "gTwo" in self.page.url:
                    return last_clicked
                await self._async_random_wait(0.25, 0.45)

            await self._async_click_first_enabled_button(["确定", "知道了", "我知道了", "继续"])
            await self._async_close_popovers()
            await self._async_random_wait(0.5, 0.8)
