...
class ZxfwInterceptMixin:
    ...
    async def _async_debug_log(self, message: str, data: Any = None) -> None:
        if hasattr(super(), "_async_debug_log"):
            await super()._async_debug_log(message, data)  # type: ignore
        else:
            self._debug_log(message, data)

    async def _async_navigate_to_url(self) -> None:
        if hasattr(super(), "_async_navigate_to_url"):
            await super()._async_navigate_to_url()  # type: ignore

    async def _async_random_wait(self, min_s: float, max_s: float) -> None:
        if hasattr(super(), "_async_random_wait"):
            await super()._async_random_wait(min_s, max_s)  # type: ignore
        else:
            import random

            await asyncio.sleep(random.uniform(min_s, max_s))

    async def _async_save_page_state(self, name: str) -> dict[str, Any]:
        if hasattr(super(), "_async_save_page_state"):
            return await super()._async_save_page_state(name)  # type: ignore
        return {}

    async def _async_download_document_directly(
        self,
        document_data: dict[str, Any],
        download_dir: Path,
        download_timeout: int,
    ) -> tuple[bool, str | None, str | None]:
        if hasattr(super(), "_async_download_document_directly"):
            return await super()._async_download_document_directly(document_data, download_dir, download_timeout)  # type: ignore
        raise NotImplementedError("子类必须实现 _async_download_document_directly 方法")

    async def _async_save_documents_batch(  # pragma: no cover
        self,
        documents_with_results: list[tuple[dict[str, Any], tuple[bool, str | None, str | None]]],
    ) -> dict[str, Any]:
        if hasattr(super(), "_async_save_documents_batch"):
            return await super()._async_save_documents_batch(documents_with_results)  # type: ignore
        return {}

    # ── sync implementations ─────────────────────────────────────

    def _intercept_api_response_with_navigation(
    ...
    async def _async_intercept_api_response_with_navigation(
        self,
        timeout: int = 30000,
        page: AsyncPage | None = None,
    ) -> dict[str, Any] | None:  # pragma: no cover
        """异步版：在导航前注册监听器，拦截 API 响应。
        ...
        intercepted_data: dict[str, Any] | None = None
        start_time = time.time()
        logger.info(f"开始拦截 API 响应(异步,导航前注册),超时时间: {timeout}ms")

        async def handle_response(response: Any) -> None:  # pragma: no cover
            nonlocal intercepted_data
            if api_url in response.url:
                try:
                    ...
                    logger.info(
                        "成功拦截 API 响应(异步)",
                        extra={
                            "operation_type": "api_intercept_async",
                            "timestamp": time.time(),
                            "document_count": document_count,
                            "response_time_ms": response_time,
                            "api_url": api_url,
        ...
        try:
            ...
            logger.info(f"已注册异步 API 响应监听器: {api_url}")
            await self._async_debug_log("开始导航到目标页面(异步)")
            await self._async_navigate_to_url()
            await self._async_debug_log("等待页面加载 (networkidle)(异步)")
            await _page.wait_for_load_state("networkidle", timeout=30000)
            await self._async_debug_log("额外等待 3 秒,确保页面完全加载(异步)")
            await self._async_random_wait(3, 5)
            if intercepted_data is None:
                timeout_seconds = timeout / 1000.0
                elapsed = 0.0
                check_interval = 0.5
                logger.info("API 响应尚未拦截到,继续等待(异步)...")
                while intercepted_data is None and elapsed < timeout_seconds:
            ...
            try:
                ...
                logger.info("已移除异步 API 响应监听器")
            except Exception as e:
                logger.warning(f"移除异步监听器失败: {e}")
        return intercepted_data

    async def _async_download_via_api_intercept_with_navigation(
        self,
        download_dir: Path,
        page: AsyncPage | None = None,
    ) -> dict[str, Any]:
        """异步版：通过 API 拦截方式下载文书（在导航前注册监听器）"""
        api_data = await self._async_intercept_api_response_with_navigation(timeout=30000, page=page)
        await self._async_debug_log("保存页面状态(异步)")
        await self._async_save_page_state("zxfw_after_navigation")
        return await self._async_process_api_data_and_download(api_data, download_dir)

    async def _async_process_api_data_and_download(
        self,
        ...
        if len(documents) == 0:
            raise ValueError("API 响应中没有文书数据")
        logger.info(
            f"成功获取文书列表(异步),共 {len(documents)} 个文书",
            extra={
                "operation_type": "api_intercept_parse_success",
                "timestamp": time.time(),
                "document_count": len(documents),
        ...
        failed_count = 0
        for i, document_data in enumerate(documents, 1):
            logger.info(f"处理第 {i}/{len(documents)} 个文书(异步): {document_data.get('c_wsmc', 'Unknown')}")
            download_result = await self._async_download_document_directly(
                document_data=document_data, download_dir=download_dir, download_timeout=60000
            )
            success, filepath, _ = download_result
            if success:
            ...
            if i < len(documents):
                ...
                delay = random.uniform(1, 2)
                logger.info(f"等待 {delay:.2f} 秒后继续下载下一个文书(异步)")
                await asyncio.sleep(delay)
        db_save_result = await self._async_save_documents_batch(documents_with_results)
        logger.info(
            "文书下载完成(异步)",
            extra={
                "operation_type": "download_summary",
        ...
        return {
            ...
            "files": downloaded_files,
            "db_save_result": db_save_result,
            "message": f"API 拦截方式(异步):成功下载 {success_count}/{len(documents)} 份文书",
        }
