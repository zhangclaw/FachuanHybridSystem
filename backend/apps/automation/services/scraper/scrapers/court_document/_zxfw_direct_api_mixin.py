...
class ZxfwDirectApiMixin:  # pragma: no cover
    ...
    async def _afetch_documents_via_direct_api(  # pragma: no cover
        self, params: dict[str, str]
    ) -> list[dict[str, Any]]:
        """异步版直接调用法院 API 获取文书列表"""
        import httpx
        ...
        payload = {"sdbh": params.get("sdbh"), "qdbh": params.get("qdbh"), "sdsin": params.get("sdsin")}
        logger.info(f"直接调用 API (async): {api_url}, payload: {payload}")
        start_time = time.time()
        ...
        if not isinstance(documents, list):
            raise ValueError(f"API 响应 data 字段格式错误: {type(documents)}")
        logger.info(f"直接 API 获取到 {len(documents)} 个文书 (async)")
        return documents

    async def _adownload_document_directly(  # pragma: no cover
        self, document_data: dict[str, Any], download_dir: Path, download_timeout: int = 60000
    ) -> tuple[bool, str | None, str | None]:
        """异步版直接下载文书文件，返回 (成功标志, 文件路径, 错误信息)"""
        start_time = time.time()
        try:
            url = document_data.get("wjlj")
            if not url:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=download_timeout / 1000.0, follow_redirects=True) as client:
    ...
    async def _adownload_via_direct_api(self, url: str, download_dir: Path) -> dict[str, Any]:  # pragma: no cover
        ...
        if len(documents) == 0:
            raise ValueError("API 返回的文书列表为空")
        logger.info(f"直接 API 获取到 {len(documents)} 个文书,开始下载 (async)")
        downloaded_files: list[str] = []
        documents_with_results: list[tuple[dict[str, Any], tuple[bool, str | None, str | None]]] = []
        success_count = 0
        failed_count = 0
        for i, document_data in enumerate(documents, 1):
            logger.info(f"下载第 {i}/{len(documents)} 个文书: {document_data.get('c_wsmc', 'Unknown')}")
            download_result = await self._adownload_document_directly(
                document_data=document_data, download_dir=download_dir, download_timeout=60000
            )
            success, filepath, _ = download_result
            if success:
