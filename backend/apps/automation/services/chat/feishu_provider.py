...
class FeishuChatProvider(FeishuTokenMixin, FeishuFileMixin, FeishuOwnerMixin, ChatProvider):  # pragma: no cover
    ...
    async def _abuild_owner_payload(self, effective_owner_id: str, payload: dict[str, Any]) -> None:  # pragma: no cover
        """异步版本。将群主 ID 写入请求体"""
        if self.owner_config.is_validation_enabled():
            try:
                self.owner_config.validate_owner_id_strict(effective_owner_id)
        ...
        if effective_owner_id.startswith("on_"):
            open_id = await self._aconvert_union_id_to_open_id(effective_owner_id)
            if open_id:
                payload["owner_id"] = open_id
                payload["user_id_list"] = [open_id]
                logger.debug(f"转换union_id为open_id: {effective_owner_id} -> {open_id}")
    ...
    async def acreate_chat(self, chat_name: str, owner_id: str | None = None) -> ChatResult:  # pragma: no cover
        ...
        try:
            ...
            if effective_owner_id:
                await self._abuild_owner_payload(effective_owner_id, payload)
