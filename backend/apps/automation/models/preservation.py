...
class PreservationQuote(LifecycleModel):
    ...
    credential_id = models.IntegerField(
        null=True, blank=True, verbose_name="凭证ID", help_text="关联的账号凭证ID(可选,系统会自动获取Token)"
    )
    status = models.CharField(
        max_length=32, choices=QuoteStatus.choices, default=QuoteStatus.PENDING, verbose_name="任务状态"
    )
...
class InsuranceQuote(models.Model):
    ...
    preservation_quote = models.ForeignKey(
        PreservationQuote, on_delete=models.CASCADE, related_name="quotes", verbose_name="询价任务"
    )
    company_id = models.CharField(max_length=64, verbose_name="保险公司ID", help_text="cId")
    company_code = models.CharField(max_length=64, verbose_name="保险公司编码", help_text="cCode")
    company_name = models.CharField(max_length=256, verbose_name="保险公司名称", help_text="cName")
    ...
    min_premium = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, verbose_name="最低收费1", help_text="minPremium"
    )
    min_amount = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, verbose_name="最低收费2", help_text="minAmount"
    )
    max_amount = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, verbose_name="最高收费", help_text="maxAmount"
    )
    min_rate = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, verbose_name="最低费率", help_text="minRate"
    )
    max_rate = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, verbose_name="最高费率", help_text="maxRate"
    )
...
class CasePreservationQuoteBinding(models.Model):
    ...
    case = models.ForeignKey(
        "cases.Case", on_delete=models.CASCADE, related_name="preservation_quote_bindings", verbose_name="案件"
    )
    preservation_quote = models.ForeignKey(
        PreservationQuote,
    ...
    preserve_amount_snapshot = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        verbose_name="绑定时保全金额",
        help_text="用于复用询价时匹配案件当前金额",
    )
