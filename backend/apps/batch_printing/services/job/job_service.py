...
from apps.batch_printing.services.execution import MacPrintExecutorService, RuleService
    ...
    def create_job(self, *, files: list[UploadedFile], created_by: Any | None = None) -> BatchPrintJob:  # pragma: no cover
        ...
        try:
            for index, upload in enumerate(files, start=1):
                ...
                if upload.size and upload.size > _MAX_FILE_SIZE:
                    raise ValidationException(
                        message=f"文件大小超过限制（最大 {_MAX_FILE_SIZE // (1024 * 1024)}MB）",
                        errors={"file": f"{original_name} 文件过大"},
                    )
