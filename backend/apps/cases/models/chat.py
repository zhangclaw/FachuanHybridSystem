...
class CaseChat(models.Model):
    ...
    id: int
    case = models.ForeignKey(Case, on_delete=models.CASCADE, related_name="chats", verbose_name="案件")
    platform = models.CharField(
        max_length=32, choices=ChatPlatform.choices, default=ChatPlatform.FEISHU, verbose_name="平台"
    )
    ...
    owner_verified = models.BooleanField(default=False, verbose_name="群主已验证", help_text="群主设置是否已验证")
    owner_verified_at = models.DateTimeField(
        null=True, blank=True, verbose_name="群主验证时间", help_text="群主设置验证成功的时间"
    )
    creation_audit_log: Any = models.JSONField(
        default=dict, verbose_name="创建审计日志", help_text="群聊创建过程的详细日志,包含群主设置信息"
    )
...
class ChatAuditLog(models.Model):
    ...
    external_chat_id = models.CharField(
        max_length=128, blank=True, verbose_name="群聊ID", help_text="群聊的外部ID,便于查询"
    )
    platform = models.CharField(
        max_length=32,
    ...
    audit_version = models.CharField(
        max_length=16, default="1.0", verbose_name="审计版本", help_text="审计日志格式版本"
    )
